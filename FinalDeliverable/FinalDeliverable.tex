...
In the last few years, automated program repair techniques have been developed with the goal of automatically generating patches for fixing bugs in software. The development of these techniques has been necessitated by the paucity of human programmers compared to the large number of bugs that need to be fixed. However, a significant number of these approaches are targeted toward fixing bugs in C programs. We want to improve the effectiveness of automated program repair techniques in fixing bugs in Java programs. In addition, current program repair techniques, particularly those that are based on the generate-and-validate approach such as GenProg, are limited by the fact that it is possible to generate candidate patches which fail to compile. In this work, we have provided a starting point toward addressing these limitations in current program repair approaches. We do this by building a probabilistic model that describes the statement replacement actions of human programmers when they fix bugs in Java programs. To build this model, we conducted a study of 100 open source Java projects in GitHub. We have evaluated this model by using a 10-fold cross-validation approach. We hope that this model can be used by existing automatic program repair tools to generate better patches for fixing bugs in Java programs.
